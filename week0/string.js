...
const number = 10;
const replaced = fullName.replace(/[aeiouyAEIOUY]/g, number);
...
const insert = (stringToUpdate, newValue, position) => {
    if (stringToUpdate.length === 0 || stringToUpdate.indexOf(position) === -1) {
        return 'Error, string is not valid';
    }
    return stringToUpdate.replace(position, newValue);
};
